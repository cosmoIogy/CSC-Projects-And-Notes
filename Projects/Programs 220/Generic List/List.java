...
class Node<grant>
{
	...
	public Node<grant> getLink()
	{
		return this.link;
	}

	public void setLink(Node<grant> link)
	{
		this.link = link;
	}
}
...
// the List class
public class List<grant>
{
	public static final int MAX_SIZE = 50;

	private Node<grant> head;
	private Node<grant> tail;
	private Node<grant> curr;
	...
	public List(List<grant> l) {
		// Create a new list
		head = tail = curr = null;
		
		//set the new node equal to the original head
		Node<grant> originalCurr = l.head;
	...
	public void InsertBefore(grant data) {
		//if not full
		if (!IsFull()) {
			//make a new node
            Node<grant> newNode = new Node<>();
	...
	// the new element becomes the current
	// this should not be possible for a full list
	public void InsertAfter(grant data) {
		//if not full
		if (!IsFull()) {
			//make a new node
			Node<grant> item = new Node<>();
			//set the data
			item.setData(data);
			...
			//if inserting in the middle of the list
				...
				Node<grant> address = curr.getLink();
	...
	public void Remove() {
		...
		if (!IsEmpty()) {
			...
			//if curr is head
				...
				Node<grant> nextElement = head.getLink();
	...
	// this should not be possible for an empty list
	public void Replace(grant data)
	{
		//if not empty
		if(!IsEmpty()){
	...
	public boolean Equals(List<grant> l)
	{
		...
		Node<grant> thisCurrent = this.head; 
		//temporary variable for the head node of the other list
		Node<grant> lCurrent = l.head;       
